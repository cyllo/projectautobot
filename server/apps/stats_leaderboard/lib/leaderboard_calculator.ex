defmodule StatsLeaderboard.LeaderboardCalculator do
  ...
  def create_leaderboard_snapshot do
    get_latest_stats()
      |> calculate_stats
  end

  defp calculate_stats([]), do: {:error, "No snapshot statistics were found"}
  defp calculate_stats([_]), do: {:error, "There's only a single snapshot statistic"}
